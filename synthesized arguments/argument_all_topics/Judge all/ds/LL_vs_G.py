...
arg1_col = 'argument_llama'
arg2_col = 'argument_gemma'
...
async def generate_argument_forlist(topic,index, filename):          
        op1 = df_arg.at[index,'op1'].replace('"', '\\"')
        op2 = df_arg.at[index,'op2'].replace('"', '\\"')
        op3 = df_arg.at[index,'op3'].replace('"', '\\"')
        opinions1 = f'"{op1}",\n"{op2}",\n"{op3}"'
        
        arg1 = df_arg.at[index,arg1_col].replace('"', '\\"')
        arg2 = df_arg.at[index,arg2_col].replace('"', '\\"')
...
async def background_task():   
    ...
    try:
        version='_v1'
        df_arg = pd.read_csv('../../master_args.csv') 
 
        filename = model_config + arg1_col + arg2_col
...
def read_configuration():
    ...
    try:
        with open('../context.txt', 'r', encoding='utf-8') as filef:
            context = filef.read()
